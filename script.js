...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===========================
    // ==== 訪問回数カウンター機能 (セッションベース) ==== // ★★★ デバッグログ追加 ★★★
    // ===========================
    if (visitorCounterHeader) {
        console.log("カウンター処理開始"); // ★デバッグログ

        // sessionStorageから訪問フラグを取得
        let hasVisitedThisSession = sessionStorage.getItem('visitedThisSession');
        console.log("hasVisitedThisSession:", hasVisitedThisSession); // ★デバッグログ

        // 累計の「セッション単位でのアクセス回数」をlocalStorageで管理
        let storedVisits = localStorage.getItem('totalSessionPageVisits');
        console.log("storedVisits (from localStorage):", storedVisits); // ★デバッグログ
        let totalSessionVisits = Number(storedVisits) || 0;
        console.log("totalSessionVisits (初期値):", totalSessionVisits); // ★デバッグログ

        if (!hasVisitedThisSession) {
            console.log("このセッションは初めての訪問です。"); // ★デバッグログ
            totalSessionVisits += 1;
            console.log("totalSessionVisits (加算後):", totalSessionVisits); // ★デバッグログ
            try {
                localStorage.setItem('totalSessionPageVisits', totalSessionVisits);
                console.log("localStorageに totalSessionPageVisits を保存しました:", totalSessionVisits); // ★デバッグログ
            } catch (e) {
                console.error("localStorageへの保存に失敗しました:", e); // ★エラーログ
            }
            try {
                sessionStorage.setItem('visitedThisSession', 'true');
                console.log("sessionStorageに visitedThisSession を保存しました: true"); // ★デバッグログ
            } catch (e) {
                console.error("sessionStorageへの保存に失敗しました:", e); // ★エラーログ
            }
        } else {
            console.log("このセッションでは既に訪問済みです。"); // ★デバッグログ
        }

        // HTMLに表示
        visitorCounterHeader.innerHTML = `<span class="counter-number">${totalSessionVisits}</span><span class="counter-text"> 回目のアクセスです！</span>`;
        console.log("カウンター表示を更新しました:", visitorCounterHeader.innerHTML); // ★デバッグログ
